...
const NavGroup = ({
  ...
  const selectedID = menuStore((state) => state.selectedID);
  const drawerOpen = menuStore((state) => state.drawerOpen);
  const activeID = menuStore((state) => state.activeID);
  ...
  const checkOpenForParent = (child: NavItemType[], id: string) => {
    child.forEach((ele: NavItemType) => {
      if (ele.children?.length && currentItem?.id) {
  ...
  const checkSelectedOnload = (data: NavItemType) => {
    ...
    children.forEach((itemCheck: NavItemType) => {
      if (itemCheck.children?.length && currentItem?.id) {
        checkOpenForParent(itemCheck.children, currentItem.id);
      }
      if (itemCheck.url === pathname && itemCheck?.id) {
        activeID(itemCheck.id);
      }
    });
  };
      ...
      case "collapse":
        return (
          <NavCollapse
            ...
            level={1}
            parentId={currentItem?.id as string}
          />
        );
      case "item":
        return (
  ...
  return (
    <>
      {menuOrientation === LAYOUT_CONST.VERTICAL_LAYOUT || downLG ? (
        <List sx={{ mt: drawerOpen && item.title ? 1.5 : 0, py: 0, zIndex: 0 }}>
          {navCollapse}
        </List>
      ) : (
        <List>
          ...
          >
            ...
            {openMini ? (
              <KeyboardArrowDownIcon
                style={{ fontSize: theme.spacing(2), stroke: "1.5" }}
              />
            ) : (
              <KeyboardArrowUpIcon
                style={{ fontSize: theme.spacing(2), stroke: "1.5" }}
