...
import { fetchOrderSupplierPO } from "api/order.api";
...
const OrderPOListPage = () => {
  ...
  useEffect(() => {
    ...
    if (selectRowIds.size > 0 && table?.getState()) {
      ...
      rows.forEach((r) => {
        if (selectRowIds.has(r.original.orderId)) {
...
export default OrderPOListPage;
