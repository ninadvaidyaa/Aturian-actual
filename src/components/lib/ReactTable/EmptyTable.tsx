...
import { useColumnFilters, useSorting, useTableActions } from "hooks/useTable";
              ...
              {isError ? (
                <div className="h-full">
                  ...
                  {filter && filter.length > 0 && (
                    <>
                      <p>Please clear filters to see data</p>
                      <p>{`Column: ${filter.map((f) => f.id).join(", ")}`}</p>
                    </>
                  )}
                  {sorting && sorting.length > 0 && (
                    <>
                      <p>Please clear Sorting to see data</p>
                      <p>{`Column: ${sorting.map((s) => s.id).join(", ")}`}</p>
                    </>
                  )}
                  <button
                   type="button"
